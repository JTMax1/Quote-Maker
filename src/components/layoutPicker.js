...
export class LayoutPicker {
    ...
    this.modalEl.innerHTML = `
      <div class="onboarding-card" style="max-width: 960px; max-height: 85vh;">
        ...
        <div class="stepper-header" style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <button class="btn-glass" id="btnCloseLayoutPicker" style="padding: 0.4rem 0.8rem;">✕</button>
        </div>
        ...
        <div style="padding: 1rem 1.5rem 0.5rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.75rem; border-bottom: 1px solid var(--border-glass);">
          ...
          <input type="text" class="form-input" id="layoutSearchInput" placeholder="🔍 Search layouts..." style="width: 200px; border-radius: 9999px; padding: 0.4rem 0.8rem;" />
        </div>
  ...
  renderCards() {
    ...
    grid.innerHTML = filtered.map(layout => {
      ...
      return `
        <div class="preset-card ${isActive ? 'active-theme' : ''}" data-id="${layout.id}" style="padding: 1.1rem; gap: 0.5rem; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 0.6rem;">
            <span style="font-size: 1.6rem;">${layout.icon}</span>
            <div>
              <div style="font-size: 0.95rem; font-weight: 700;">${layout.name}</div>
              <span class="brand-badge" style="font-size: 0.65rem; padding: 0.1rem 0.4rem;">${layout.category}</span>
            </div>
          </div>
          <p style="font-size: 0.78rem; color: var(--text-secondary); line-height: 1.4; margin: 0.25rem 0;">
            ${layout.description}
          </p>
          <button class="btn-apply-theme" data-id="${layout.id}" style="align-self: flex-start; margin-top: 0.25rem;">
